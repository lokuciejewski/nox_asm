                    ...
                    if let Some(target_reg_token) = tokenised_line.get(2) {
                        if target_reg_token._type == TokenType::Register {
                            match (
                                ...
                                ("A", "B") => {
                                    instruction.opcode = Some(Opcode::AND_A_B);
                                    Ok(vec![instruction])
                                }
                                ("B", "A") => {
                                    instruction.opcode = Some(Opcode::AND_B_A);
                                    Ok(vec![instruction])
                                }
                                _ => {
                                    return Err(anyhow!(
                ...
                "AB" => {
                    if let Some(target_reg_token) = tokenised_line.get(2) {
                        instruction.opcode = Some(match target_reg_token._type {
                            TokenType::Address => Opcode::AND_AB_ABSOLUTE,
                            TokenType::ImmediateValue16 => Opcode::AND_AB_IMMEDIATE,
                            _ => {
                                return Err(anyhow!(
